...
export default function StockTable({ stock }) {
  return (
    <div className="overflow-x-auto max-h-[75vh] rounded-xl border border-gray-800 shadow-lg bg-white">
            ...
            <React.Fragment key={brand.brand}>
              ...
              <tr className="sticky top-[37px] z-10 bg-gray-300">
                <td colSpan={SHADES.length + 1} className="py-1 text-center">
                  <span
                    className="inline-block px-3 sm:px-15 py-1 rounded-lg border border-gray-400
                      bg-gradient-to-bl from-[#0f172a] via-[#1e1a78] to-[#0f172a]
                      text-indigo-100 font-bold tracking-wider shadow-xl shadow-gray-600/30 text-xs sm:text-base"
                  >
                    {brand.brand}
                  </span>
                </td>
              ...
              {SIZES.map(size => (
                <tr key={size} className="hover:bg-gray-50 transition-colors">
                  <td className="sticky text-center left-0 bg-white px-2 sm:px-3 py-1 sm:py-2 border border-gray-500/30 text-gray-700 font-semibold tracking-wider">
                    {size}
                  </td>
                  ...
                  {SHADES.map(shade => {
                    ...
                    let cls = "bg-emerald-200 text-emerald-800 font-semibold";
                    if (qty === 0) cls = "bg-red-200 text-red-900 font-bold";
                    else if (qty <= min) cls = "bg-amber-200 text-amber-900 font-semibold";

                    return (
                      <td
                        key={shade}
                        className={`${cls} px-2 sm:px-3 py-1 sm:py-2 font-saira font-bold tracking-wider border border-gray-500/20 text-center`}
                      >
                        {qty}
                      </td>
                    );
